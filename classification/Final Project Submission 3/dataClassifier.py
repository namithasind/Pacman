...
#    print count
    for x in height:
        ...
        for y in width:
            ...
            white = k > 0
            black = k == 0
            condition1 = feature == 0 and white
            auxCond = feature == 1 and black
            feature1 = pix == 0 and auxCond
            condition2 = feature == 1 and pix == 1 and white
            feature2 = pix == 1 and otherFeat == 1 and auxCond
#            print "condition1", condition1, "auxCond", auxCond
            if condition1:
                feature = 1
            if feature1:
                pix = 1
...
def enhancedPacmanFeatures(state, action):
    ...
    for food in foodList:
        minDist = min( minDist, util.manhattanDistance(pacmanPos, food) )

    if features["nearestFood"] == 0:
        pass
    
    if minDist != float("inf"):
        features["nearestFood"] = 1.0 / minDist
    else:
        features["nearestFood"] = 2

    
    minDist = float("inf")
    for ghost in ghostPos:
        minDist = min(minDist, util.manhattanDistance(pacmanPos, ghost))
    
    features["neatestGhost"] = minDist
