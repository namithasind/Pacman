...
#        util.raiseNotDefined()
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            touchwall = self.walls[nextx][nexty]
            if not touchwall:
                svisitedcorners = list(visitedcorners)
                next_node = (nextx, nexty)
                if next_node in self.corners:
                    if not next_node in svisitedcorners:
                        svisitedcorners.append( next_node )
                successor = ((next_node, svisitedcorners), action, 1)
...
def cornersHeuristic(state, problem):
    ...
    while len(unvisitedCorners) > 0:
        distance, corner = min([(util.manhattanDistance(curr, corner), corner) for corner in unvisitedCorners])
        h = h + distance
        curr = corner
...
class FoodSearchProblem:
    ...
    def getSuccessors(self, state):
        "Returns successor states, the actions they require, and a cost of 1."
        successors = []
#        print "stat   food", state[0], state[1]
        self._expanded += 1 # DO NOT CHANGE
...
#    if len(foodlist) == 0:
#        return 0
#    distance = max([(util.manhattanDistance(position, food)) for food in foodlist])
#    h = distance - h
#    return h
    position, foodGrid = state
    h = 0
    ...
    if(max_distance[0]==(0,0) and max_distance[1]==(0,0)):
        h = util.manhattanDistance(position,foodlist[0])
    else:
