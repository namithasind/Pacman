    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            for count in range(self.numParticles):
                self.particles[count] = self.getJailPosition()
            ...
            if allPossible.totalCount() == 0:
                ...
                for count in range(self.numParticles):
                    self.particles[count] = util.sample(allPossible)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        permutations = list(itertools.product(legalpos, repeat=self.numGhosts))
        # print len(permutations)
        random.shuffle(permutations)
        while num >= len(permutations):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if i not in jailGhosts:
                    ...
                    probability *= em[distance]
            
            newParticles[particle] += probability
